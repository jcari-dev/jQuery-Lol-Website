$(() => {
  ...
  let $api_key = "?api_key="; // my api key
  ...
  let $championImg =
    "https://ddragon.leagueoflegends.com/cdn/11.7.1/img/champion/";
  ...
  let $playerIconUrl =
    "https://ddragon.leagueoflegends.com/cdn/11.7.1/img/profileicon/"; // SELF
  ...
  let $eggs = () => {
    // first ajax request, everything begins with your summoner name, since it's the only public info that you have before this call
    $call = $.ajax({
      // INPUT SUMMONER NAME GET ACCOUNT ID
      type: "GET",
      ...
      $.getJSON(
        "https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/champion.json",
        function (championList) {
          // json provided by api developer, it contains a list of all champions nested in an object
          $championList = championList;
        }
      );
      $.getJSON(
        "https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/item.json",
        function (itemList) {
          $daListItem = Object.entries(itemList.data);
        }
      );
      $.getJSON(
        "https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/profileicon.json",
        function (iconList) {
          $daListIcon = iconList;
        }
      );
      $.getJSON(
        "https://ddragon.leagueoflegends.com/cdn/11.7.1/data/en_US/summoner.json",
  ...
  let $pepper = () => {
    $.ajax({
      // once you get the encrypted account id, then you can call for match history
      type: "GET",
      ...
      for (let i = 0; i < $numgames; i++) {
        // this for loop basically defines the number of matches that will be displayed
  ...
  let $milk = () => {
    // console.log('hi');
    $.ajax({
      // if the user is !== unranked then throw the following ajax call to get users rank info
      type: "GET",
      ...
      for (let q of data5) {
        // used for/of loop just to try to see if i could optimize the code
  ...
  let $bread = () => {
    $.ajax({
      // if the user is !== unranked then throw the following ajax call to determine the winrate of the user in the same tier and division
      type: "GET",
      url:
        $magic +
        $http +
  ...
  let $ham = () => {
    // ham goes brrr, basically this call gets all the information in regards to the match, this call is executed the number of times that the user request on contrary to the others which are only needed once
    $accountInfoAppend();
    for (let p = 0; p < $matchesId.length; p++) {
      ...
      $.ajax({
        type: "GET",
        url:
          $magic + $http + $server + $baseurlMatchId + $matchesId[p] + $api_key,
      }).then(function (data3) {
        console.log(data3);
        console.log(
          $http + $server + $baseurlMatchId + $matchesId[p] + $api_key
        );
        $individualMatchId = [p];
        // console.log(data3) NEEEEEDEDEDEDEDEDEDEDE
        for (let j = 0; j < 10; j++) {
          if (
            data3.participantIdentities[j].player.currentAccountId == $accid
          ) {
            $playerPosition = [j];
        ...
        if (data3.participants[$playerPosition].stats.win == true) {
          // sets if the game was won or lost
          $gResult = "Victory!";
        } else {
          $gResult = "Defeat!";
        }

        if (data3.participants[$playerPosition].stats.firstBloodKill == true) {
          // sets if the game was won or lost
        ...
        for (let s = 0; s < 14; s++) {
          if (
            data3.participants[$playerPosition].spell1Id ==
            $summlocationjson[s][1].key
          ) {
          ...
          if (
            data3.participants[$playerPosition].spell2Id ==
            $summlocationjson[s][1].key
          ) {
        ...
        $totalMagicDmgDealt =
          data3.participants[
            $playerPosition
          ].stats.magicDamageDealt.toLocaleString();
        $totalMagicDmgDealtToChamp =
          data3.participants[
            $playerPosition
          ].stats.magicDamageDealtToChampions.toLocaleString();
        $totalPhyDmgDealt =
          data3.participants[
            $playerPosition
          ].stats.physicalDamageDealt.toLocaleString();
        $totalPhyDmgDealtToChamp =
          data3.participants[
            $playerPosition
          ].stats.physicalDamageDealtToChampions.toLocaleString();
        $totalTrueDmgDealt =
          data3.participants[
            $playerPosition
          ].stats.trueDamageDealt.toLocaleString();
        $totalTrueDmgDealtToChamp =
          data3.participants[
            $playerPosition
          ].stats.trueDamageDealtToChampions.toLocaleString();
        // $killingSprees = data3.participants[$playerPosition].stats.killingSprees
        $longestTimeSpentLiving =
          data3.participants[$playerPosition].stats.longestTimeSpentLiving;
        ...
        $timeApplyingCC =
          data3.participants[$playerPosition].stats.timeCCingOthers;
        $totalDmgDealt =
          data3.participants[
            $playerPosition
          ].stats.totalDamageDealt.toLocaleString();
        $totalDmgDealtToChampion =
          data3.participants[
            $playerPosition
        ...
        $totalHeal =
          data3.participants[$playerPosition].stats.totalHeal.toLocaleString();
        $totalTimeCced =
          data3.participants[$playerPosition].stats.totalTimeCrowdControlDealt;
        ...
        $goldSpent =
          data3.participants[$playerPosition].stats.goldSpent.toLocaleString();
        $firstBloodKill =
          data3.participants[$playerPosition].stats.firstBloodKill; //bool
        $firstBloodAssist =
          data3.participants[$playerPosition].stats.firstBloodAssist; //
        $dmgSelfMitigated =
          data3.participants[$playerPosition].stats.damageSelfMitigated;

        $("#rank").text(
          `Winrate of the last ${$recentMatches} games, ${$winrate}%`
        );
        ...
        $gameDateLocation = new Date($gameCreationDate).toLocaleDateString(
          "en-us"
        );
        ...
        $items = Object.values(
          Object.values(
            Object.entries(data3.participants[$playerPosition].stats)
          )
        );
        for (let x = 0; x < $daListChamp.length; x++) {
          if (
            data3.participants[$playerPosition].championId ==
            $daListChamp[x].key
          ) {
            $championPosition = x;
        ...
        for (let r = 2; r < 9; r++) {
          ...
          if ($items[r][1] >= 7000 && $items[r][1] < 7023) {
            // check for ornn items which arent in the database and use external source to append them
  ...
  $accountInfoAppend = () => {
    ...
    $("#accountinfo").append(
      `<img class="usericon" src=${$playerIconFullUrl}><br>`
    );
    $("#accountinfo").append(
      `IGN: ${$sname2} <br> Summoner Level: ${$slevel}<br>`
    );
    ...
    if ($userunranked == "Unranked") {
      ...
      $("#accountinfo").append(
        `<br>Average ${$rank} ${$division} Winrate: ${$tierWinrate}%<br>`
      );
    }
    $("#accountinfo").show();
  };
  ...
  $appendItems = () => {
    // console.log("hi")
    if ($participantXitems0to6[$itemr][$itemnum] == 0) {
    } else if ($ornnItemPresent == true) {
      $(`#match${$individualMatchId}`).append(
        `<img class="itemImg tooltip" src="${$ornnItems}${$items[$itemr][$itemnum]}.png">`
      );
    } else if ($ornnItemNotPresent == true) {
      $(`#match${$individualMatchId}`)
        .append(`<div class="tooltip"><a href="https://leagueoflegends.fandom.com/wiki/${$wikiaItem}" target="_blank">
